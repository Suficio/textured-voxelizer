...
pub fn rgb2hsv(rgb: Vector4::<u8>) -> Vector4::<f32> {
    ...
    if h < 0f32 { h += 2f32 * std::f32::consts::PI }
...
pub fn hsv2rgb(hsv: Vector4::<f32>) -> Vector4::<u8> {
    ...
    Vector4::new(((r + m)*255f32) as u8, ((g + m)*255f32) as u8, ((b + m)*255f32) as u8, (hsv[3]*255f32) as u8)
}
...
pub fn match_hsv_to_colorset(colorset: &Vec<Vector4::<f32>>, color: &Vector4::<f32>) -> usize {
    let mut min = 0;
    let mut min_distance = hsv_distance(&colorset[0], &color);
    for i in 1..colorset.len() {
        let distance = hsv_distance(&colorset[i], &color);
